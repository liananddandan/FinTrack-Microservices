...
import { submitProcurement } from "../api/transactions";
...
async function handleSubmitProcurement() {
    ...
    try {
        await submitProcurement(detail.value.transactionPublicId);
        ElMessage.success("Procurement submitted successfully.");
        await load();
    } catch (err: any) {
        ElMessage.error(err.message || "Failed to submit procurement.");
    } finally {
            ...
            <el-card v-loading="loading" class="summary-card" shadow="never">
                <template v-if="detail">
                    <div class="summary-top">
                        ...
                        <div v-if="detail && detail.type === 'Procurement' && detail.status === 'Draft'"
                            class="action-bar">
                            <el-button type="primary" :loading="actionLoading" @click="handleSubmitProcurement">
                                Submit for Approval
                            </el-button>
                        </div>
                    </div>
