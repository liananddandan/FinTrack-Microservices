import { getCurrentUser, selectPlatform, type UserProfile } from "../api/account"
...
export const platformAuthStore = {
  ...
  async activatePlatformContext(): Promise<boolean> {
    try {
      const result = await selectPlatform()
      this.setPlatformAccessToken(result.platformAccessToken, result.platformRole)
      return true
    } catch {
      this.clearPlatformAccessToken()
      return false
    }
  },
